...
int8_t TM_USART_Getc(USART_TypeDef* USARTx, uint8_t* c) {
	int8_t result = 0;
	*c=0;
	...
	if (u->Num > 0) {
		...
		if (u->Out == u->Size) {
			u->Out = 0;
			//trace_printf("Buffer overflow USART");
		}
...
uint16_t TM_USART_Gets(USART_TypeDef* USARTx, char* buffer, uint16_t bufsize) {
	...
	while (i < (bufsize - 1)) {
		...
		USART_result = TM_USART_Getc(USARTx,buffer[i]);
		if (!USART_result)  {
		   break;}
...
#if defined(DEBUG)
	else  trace_printf("overflow IRQhandler\n");
#endif
}
...
void USART1_IRQHandler(void) {
	...
	if (USART1->SR & USART_SR_RXNE) {
		#if defined(TM_USART1_USE_CUSTOM_IRQ)
			/* Call user function */
...
void USART2_IRQHandler(void) {
	...
	if (USART2->SR & USART_SR_RXNE) {
		#if defined(TM_USART2_USE_CUSTOM_IRQ)
			/* Call user function */
...
void USART3_IRQHandler(void) {
	...
	if (USART3->SR & USART_SR_RXNE) {
		#if defined(TM_USART3_USE_CUSTOM_IRQ)
			/* Call user function */
...
void UART4_IRQHandler(void) {
	...
	if (UART4->SR & USART_SR_RXNE) {
		#if defined(TM_UART4_USE_CUSTOM_IRQ)
			/* Call user function */
...
void UART5_IRQHandler(void) {
	...
	if (UART5->SR & USART_SR_RXNE) {
		#if defined(TM_UART5_USE_CUSTOM_IRQ)
			/* Call user function */
...
void USART6_IRQHandler(void) {
	...
	if (USART6->SR & USART_SR_RXNE) {
		#if defined(TM_USART6_USE_CUSTOM_IRQ)
			/* Call user function */
...
void UART7_IRQHandler(void) {
	...
	if (UART7->SR & USART_SR_RXNE) {
		#if defined(TM_UART7_USE_CUSTOM_IRQ)
			/* Call user function */
