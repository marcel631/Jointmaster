...
//short lulcount;
//char lulBuf[APP_READ_BUFFER_SIZE];
...
TM_USB_VCP_Result TM_INT_USB_VCP_AddReceived2(uint8_t* c, uint32_t Len) {
	/* Still available data in buffer */
	uint32_t i;
	if (tm_int_usb_vcp_buf_num + Len < USB_VCP_RECEIVE_BUFFER_LENGTH) {
		/* Check for overflow */
		CpuCriticalVar();
		CpuEnterCritical();
		for (i = 0; i < Len; i++) {
	...
	/* Return Buffer full */
        ...
        trace_printf("Receive buffer full");
#endif
